# Khóa học ASP.NET, Web API
(đã hoàn thành)

Nội dung các video của khóa học có thể xem tại đây: https://www.youtube.com/playlist?list=PLRLJQuuRRcFnwlQxGeVSVv-z_5tFwAh0j

## Mục tiêu
- Hiểu rõ cách vận hành của một phần mềm ASP.NET.
- Biết cách đọc tài liệu và tự học các công nghệ khác trên .NET.
- Có khả năng tham gia vào các dự án thực tế sử dụng ASP.NET.
- Biết cách tổ chức một dự án nhỏ với các công nghệ đã học trong khóa học.

## Tổng quan chương trình
Khóa học sẽ bao gồm các nhóm nội dung cơ bản sau:
- Căn bản về ASP.NET MVC, Web API.
- Thiết kế và triển khai ứng dụng ASP.NET.
- Bài tập áp dụng các kiến thức đã học, đồng thời kết hợp tìm hiểu về các design pattern.
- Tối ưu và best practices với ứng dụng web.

## Nội dung

- ASP.NET - Phần 1 - Giới thiệu về ASP.NET và viết chương trình đầu tiên
- ASP.NET - Phần 2 - Controller và action method
- ASP.NET - Phần 3 - Các Attribute thường dùng
- ASP.NET - Part 4 - Routing
- ASP.NET - Phần 5 - Bài tập: Viết chương trình To Do List
- ASP.NET - Part 6 - HttpContext
- ASP.NET - Phần 7 - Làm việc với HttpRequest
- ASP.NET - Phần 8 - Làm việc với HttpResponse
- ASP.NET - Phần 9 - Bài tập: clone lại tính năng Session của 
- ASP.NET - Phần 10 - Bài tập: Nâng cấp ứng dụng MySession và thêm integration test
- ASP.NET - Phần 11 - Configuration và Options pattern
- ASP.NET - Part 12 - Introduction to Middleware
- ASP.NET - Phần 13 - Thực hành viết một middleware
- ASP.NET - Phần 14 - Model binding
- ASP.NET - Part 15 - Session
- ASP.NET - Phần 16 - Https Redirection
- ASP.NET - Phần 17 - Giới thiệu các khái niệm trong ...
- ASP.NET - Phần 18 - Chương trình đầu tiên với authentication và authorization
- ASP.NET - Phần 19 - Authentication với Json Web Token (JwtBearer)
- ASP.NET - Phần 20 - Role-based Authorization
- ASP.NET - Phần 21 - Policy-based Authorization
- ASP.NET - Phần 22 - Giải bài tập: viết một OpenIdConnect ...
- ASP.NET - Phần 23 - Giải bài tập: viết một OpenIdConnect server, phần 2
- ASP.NET - Phần 24 - Sử dụng CORS
- ASP.NET - Phần 25 - Rate Limiting
- ASP.NET - Phần 26 - Sử dụng cache trong ASP.NET
- ASP.NET - Phần 27 - Làm quen với Web API
- ASP.NET - Phần 28 - Viết test cho API
- ASP.NET - Phần 29 - Bài tập: Ghtk Api - Tạo authentication handler để xác thực request
- ASP.NET - Phần 30 - Bài tập: Ghtk Api - Xác thực Token dưới ...
- ASP.NET - Phần 31 - Bài tập Ghtk Api - Xác thực với client trong SQL Server
- ASP.NET - Phần 32 - Bài tập: Ghtk Api - Xây dựng bộ khung cho các hàm API
- ASP.NET - Phần 33 - Bài tập: Ghtk Api - xây dựng repository và kết nối lại với nhau
- ASP.NET - Part 34 - REST API Design Principles
- ASP.NET - Phần 35 - Làm quen với Minimal API
- ASP.NET - Phần 36 - Parameter binding trong Minimal API
- ASP.NET - Phần 37 - Bài tập viết Core Banking API (1) - Xây dựng cấu trúc dự án với .NET Aspire
- ASP.NET - Part 38 - Core Banking API Writing Exercises (2)
- ASP.NET - Phần 39 - Core Banking API (3) - Định nghĩa các model Entity Framework
- ASP.NET - Phần 40 - Core Banking API (4) - Viết migration service để khởi tạo cơ sở dữ liệu
- ASP.NET - Phần 41 - Core Banking API (5) - Viết hàm API cho customer
- ASP.NET - Phần 42 - Core Banking API (6) - Viết các hàm ...
- ASP.NET - Phần 43 - Core Banking API (7) - Unit Tests
- ASP.NET - Phần 44 - Core Banking API (8) - Integration Tests
- ASP.NET - Phần 45 - Giới thiệu các khái niệm về ảo hóa, container và Docker
- ASP.NET - Part 46 - Deploy with Docker Compose
  
